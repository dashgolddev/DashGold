DashGold-qt: Qt4 GUI for DashGold
===============================
...
Alternatively, install Qt Creator and open the `DashGold-qt.pro` file.

An executable named `DashGold-qt` will be built.
...
A warning for people using the *static binary* version of DashGold on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of DashGold is linked against libdb5.3.
...
installed causes DashGold-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
