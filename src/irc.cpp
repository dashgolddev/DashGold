...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #DashGoldTEST2\r");
            Send(hSocket, "WHO #DashGoldTEST2\r");
        } else {
            // randomly join #DashGold00-#DashGold05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #DashGold%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #DashGold%02d\r", channel_number).c_str());
        }
